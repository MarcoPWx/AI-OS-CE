...
// Initialize MSW addon for Storybook (uses its own service worker)
initialize({ onUnhandledRequest: 'bypass' });

// Optional: patch fetch to inject mock headers used by your MSW handlers
function ensurePatchedFetch() {
  ...
  window.fetch = (input: RequestInfo | URL, init: RequestInit = {}) => {
    ...
    if (w.__mswLatencyMs && String(w.__mswLatencyMs) !== '0') headers.set('x-mock-delay', String(w.__mswLatencyMs));
    if (w.__mswErrorRate && String(w.__mswErrorRate) !== '0') headers.set('x-mock-error-rate', String(w.__mswErrorRate));
  ...
  docsQuickLink: {
    ...
    toolbar: {
      ...
      items: [
        ...
        { value: 'docs-labs-index--docs', title: 'Labs Index' },
        { value: 'docs-labs-lab-01-notification-center--docs', title: 'Lab 01: Notifications' },
        { value: 'docs-guides-mdx-features--docs', title: 'MDX Features' },
        { value: 'docs-guides-patterns-overview--docs', title: 'Patterns Overview' },
        { value: 'docs-guides-hooks-query-mutation--docs', title: 'Hooks (Query & Mutation)' },
        { value: 'docs-labs-lab-02-error-first-api--docs', title: 'Lab 02: Error-first API' },
        { value: 'docs-labs-lab-03-accessibility-audit-fixes--docs', title: 'Lab 03: A11y Audit' }
      ],
    },
  },
};
...
const preview: Preview = {
  parameters: {
    ...
    // Provide default MSW handlers for all stories; individual stories can override
    msw: { handlers },
  ...
  decorators: [
    ...
    (Story: any, context: any) => {
      const Shell: React.FC = () => {
        ...
        // Double-press 'g' within 400ms to toggle overlay; Esc closes
        useEffect(() => {
          let lastG = 0;
        ...
        // Custom event to toggle/present overlay from docs components
        useEffect(() => {
        ...
        const quickLink = useMemo(() => (quickId && quickId !== 'none' ? `?path=/docs/${quickId}` : null), [quickId]);

        return (
          <div style={{ position: 'relative' }}>
            {quickLink ? (
              <a href={quickLink} style={{ position: 'fixed', top: 10, left: 10, background: '#111', color: '#fff', padding: '6px 8px', borderRadius: 6, zIndex: 1001, textDecoration: 'none', fontSize: 12 }}>Open Doc</a>
            ) : null}
            <main role="main"><Story /></main>
            <StatusPills />
            {help ? <HelpPanel link={help} /> : null}
            <PresenterOverlay visible={overlay} onClose={() => setOverlay(false)} mswStatus={mswStatus} script={(window as any).__presenterScript} />
