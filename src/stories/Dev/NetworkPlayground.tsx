...
export default function NetworkPlayground(): JSX.Element {
  const [tests, setTests] = useState<NetworkTest[]>([
    { id: '1', name: 'Health Check', endpoint: '/api/health', method: 'GET', status: 'idle' },
    { id: '2', name: 'Database Ping', endpoint: '/api/db/ping', method: 'GET', status: 'idle' },
    { id: '3', name: 'CDN Response', endpoint: 'https://cdn.example.com/test', method: 'HEAD', status: 'idle' },
    { id: '4', name: 'WebSocket Echo', endpoint: 'wss://ws.example.com/echo', method: 'WS', status: 'idle' },
    { id: '5', name: 'GraphQL Query', endpoint: '/graphql', method: 'POST', status: 'idle' }
  ]);

  const [connections] = useState<ConnectionStatus[]>([
    { service: 'Primary API', status: 'connected', latency: 23, packetLoss: 0 },
    { service: 'Database Cluster', status: 'connected', latency: 8, packetLoss: 0.1 },
    { service: 'Redis Cache', status: 'connected', latency: 2, packetLoss: 0 },
    { service: 'CDN Edge', status: 'connecting', latency: 45, packetLoss: 0.5 },
    { service: 'Message Queue', status: 'connected', latency: 15, packetLoss: 0 }
  ]);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{ fontSize: 18, fontWeight: 600, marginBottom: 20, color: '#334155' }}>
          Bandwidth Monitor
        </h2>
            ...
            {tests.map(test => (
              ...
              >
                <div>
                  ...
                  {test.latency && (
                    <div style={{ fontSize: 11, color: getLatencyColor(test.latency), marginTop: 2 }}>
                      {test.latency}ms
                    </div>
                  )}
                </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 24 }}>
          <div>
            <label style={{ fontSize: 14, color: '#64748b', marginBottom: 8, display: 'block' }}>
              Concurrent Requests
            </label>
            <input
              type="number"
